...
public class MH4UActivity extends AppCompatActivity {

    private static final String TAG = "";
    private final String JSON_URL = "https://raw.githubusercontent.com/HukanloS/mh_info_monster_names/master/mh4uMonsterNames_en_jp_fr.json";
    ...
    private void jsonrequest() {
        ...
        requestQueue = Volley.newRequestQueue(MH4UActivity.this);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()){
            case R.id.download:
                ...
                try{
                    URL uri = new URL("https://raw.githubusercontent.com/HukanloS/mh4u_info_monster_names/master/mhMonsterNames_en_jp_fr.json");
                    HttpURLConnection conn = (HttpURLConnection) uri.openConnection();
                    conn.setRequestMethod("GET");
                    conn.connect();
                    if(HttpURLConnection.HTTP_OK == conn.getResponseCode()){
                        copyInputStreamToFile(conn.getInputStream(),new File(getCacheDir(),"mh4u_monster.json"));
                        Log.d(TAG, "Json Table <mh4u_monster.json> downloaded");
                    }
                } catch (IOException e) {
                    e.printStackTrace();
                }
                return true;
            case R.id.info:
                Uri uri = Uri.parse("http://fr.mogapedia.wikia.com/wiki/MH4U_-_Monstres");
