...
public class MHXXActivity extends AppCompatActivity {

    private static final String TAG = "";
    private final String JSON_URL = "https://raw.githubusercontent.com/HukanloS/mh_info_monster_names/master/mhxxMonsterName_en_jp_fr.json";
    ...
    private void jsonrequest() {
        ...
        requestQueue = Volley.newRequestQueue(MHXXActivity.this);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()){
            case R.id.download:
                ...
                try{
                    URL uri = new URL("https://raw.githubusercontent.com/HukanloS/mhxx_info_monster_names/master/mhMonsterName_en_jp_fr.json");
                    HttpURLConnection conn = (HttpURLConnection) uri.openConnection();
                    conn.setRequestMethod("GET");
                    conn.connect();
                    if(HttpURLConnection.HTTP_OK == conn.getResponseCode()){
                        copyInputStreamToFile(conn.getInputStream(),new File(getCacheDir(),"mhxx_monster.json"));
                        Log.d(TAG, "Json Table <mhxx_monster.json> downloaded");
                    }
                } catch (IOException e) {
                    e.printStackTrace();
                }
                return true;
            case R.id.info:
                Uri uri = Uri.parse("http://fr.mogapedia.wikia.com/wiki/MHGen_-_Monstres");
